...
15. **Full Code section must always match the step-by-step walkthrough**: The "Full Code" block in the MD is a copy-pasteable version of the exact same logic. Any change to the step-by-step functions must be reflected in Full Code — same function signatures, same logic, same variable names. The only allowed simplifications are removing verbose logging/comments that are already explained in the walkthrough. Never let the two diverge.
16. **Keep each step's responsibility narrow**: A balance-checking step (Step 1) should only report the total balance per chain — it should not filter by token type or detect non-USDC holdings. Token detection and normalisation belong in the step that actually acts on them (e.g., the swap step). Mixing concerns makes the flow harder to follow and harder to skip optional steps.
